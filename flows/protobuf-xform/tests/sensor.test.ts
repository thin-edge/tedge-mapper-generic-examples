...
test("Encode and decode sensor data", () => {
  ...
  const encodedMessage = messages.sensorpackage.SensorMessage.encode(input).finish().toString();
  const decodedMessage = messages.sensorpackage.SensorMessage.decode(Buffer.from(encodedMessage, 'utf8'));
