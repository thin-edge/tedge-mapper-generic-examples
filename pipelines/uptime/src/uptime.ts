...
export class UptimeTracker {
  ...
  private getWindowedHistory(startTime: number, endTime: number): StatusChange[] {
    ...
    if (i > 0 && this.history[i - 1].timestamp < startTime) {
      history.push({ status: this.history[i - 1].status, timestamp: startTime });
    } else if (i === 0 && this.history.length > 0 && this.history[0].timestamp > startTime) {
    ...
    for (; i < this.history.length && this.history[i].timestamp <= endTime; i++) {
